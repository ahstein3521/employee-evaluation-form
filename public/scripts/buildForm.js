...
const getSection = () => {
  ...
  return function(step = 0){
    ...
    let i = Sections[x]
    let j = Subsections[x][y]
    let q = questions[i][j].questions;
    let header = romanNums[x] + i;
    return {questions:q, section:header, subsection:j};
  }
}

const buildSection = ({questions, section, subsection}) =>{
  
  const wrapper = `<div class = 'section active'>`
  const textArea =  "<div class='form-group'><label>Please enter any comments in the space below below</label><textarea name='comments' class='form-control'></textarea></div>"
  const header = `<h4><em>${subsection}</em></h4><hr>`;
 
  let HTML = questions.reduce((acc, question) =>{ return acc+ radioHTML(question)}, header) + textArea;

  $("#section-heading").text(section);

  $(".multipleChoice").append(wrapper + HTML +"</div>");
}

var getScoreForSection = () => {
  let arr = [];
  const textarea = $(".active").find("textarea");


  $(".active").find("input:checked").each(function(){
      arr.push({name:$(this).attr("name"),value:$(this).val()})
  }); //function emulates jq method 'serializeArray'
      //, but is applied to a single section of a form rather than the entire form

  arr.push({name:"comments", value: textarea.val() || "N/A"});
    //push the value of the current textarea's value to the array

  let maxPossible = 0;

  let sectionScore = arr.reduce((acc, {name,value}) => {
      acc[name] = value;
      if(name!= "comments"){
        let parsed = value.split("/");
          acc.points += +parsed[0];
          maxPossible+= +parsed[1];
      }//if input value is a score, parse the value and calculate section score as well as total possible
      else{
        acc.score = (acc.points/maxPossible) * 10;
        acc.review = evaluateScore(acc.score);
      }//else every thing has been totalled so perform final calculations 
      return acc;
  },{points:0})
...
var evaluateScore = score => {
  const grades = [{review:"Extraordinary Performance", min:9.01, max:11.05},
                  {review:"Above Average Performance", min:6.67, max:9.0},
                  {review:"Average Performance", min:4.33, max:6.66},
                  {review:"Below Average Performance", min:2.01, max:4.32},
                  {review:"Needs Significant Improvement", min:0, max:2.0}]

 let grade = grades.find(({min,max}) => {
...
const scoreTracker = () => {
  ...
  return function({section,subsection}){
    let update = getScoreForSection();
    Score[section] = Score[section] || {};
    Score[section][subsection] = update;
    console.log(Score);
  }
}
...
//Validate form section and update css to display invalid areas
var isValid = () => {
  
  const multipleChoice = $(".active").children(".form-group");
  let scrollTarget = null;

  if($(".radio-invalid").length) return false;
    //break immediately if questions already marked invalid

  multipleChoice.each(function(){
    const radio = $(this).children("div.radio");

    if(radio.length && !radio.find("input:checked").length){
      $(this).addClass("radio-invalid");
      
      if(!scrollTarget){
        scrollTarget = $(this).offset().top - 30;
        smoothScroll(scrollTarget)
      }//scroll to topmost invalid question
    }
  })//search for existence of checked radio inputs

  return !$(".radio-invalid").length 
  //if no invalid form-group divs, return true
}

$(document).ready(function(){
  
  const getNewSection = getSection();
  const updateScore = scoreTracker(); 

  let current = getNewSection()

  buildSection(current);
  ...
  $(window).on("load",function(){
   ...
   $(".next").on("click",function(e){
    ...
    if(isValid()){
      ...
      current = getNewSection(1);

      buildSection(current);
    }else{
      $(".radio-invalid").on("change",function(){
        $(this).removeClass("radio-invalid")
      })
    }
  })
